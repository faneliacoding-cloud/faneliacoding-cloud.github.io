...
`:e>0?r+=`
${n.includes("Ms.")?"She":n.includes("Mr.")?"He":"They"} scaled the following ${s.length} out of ${t.length} symptoms at ${e===o-2?"the third most severe":"a lower"} level, indicating that they bother ${n.includes("Ms.")?"her":n.includes("Mr.")?"him":"them"} "${l}" in ${n.includes("Ms.")?"her":n.includes("Mr.")?"his":"their"} day-to-day functioning:
`:r+=`
The following ${s.length} symptoms ${n.includes("Ms.")?"she":n.includes("Mr.")?"he":"they"} experience${n.includes("They")?"":"s"} "${l}":
`,s.forEach(e=>{r+=`${e} `}),r+="\n"}return r}function f(e){let i,f,y=e.client,$=e.sections?.step01||{};e.sections?.step02;let v=e.sections?.step03||{},w=e.sections?.step04||{},b=e.sections?.step05||{},S=e.sections?.step06||{},A=e.sections?.step07||{},C=e.sections?.step08||{},T=e.sections?.step10||{},_=o(y.pronouns),D=(i=o(y.pronouns),f=y.fullName?.split(" ").pop()||"XXX",i.title?`${i.title} ${f}`:f),x=y.fullName||"[Client Name]",I=_.subject.charAt(0).toUpperCase()+_.subject.slice(1),E=_.subject,L=_.object,N=_.possessive,R=_.possessive.charAt(0).toUpperCase()+_.possessive.slice(1);_.reflexive;let O=y.countryOfOrigin||"CountryXXX",P=t.CASE_TYPE_CONFIG[e.caseType]?.label||e.caseType,H=new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"}),M=S.phq9Scores||Array(9).fill(-1),F=S.gad7Scores||Array(7).fill(-1),k=S.pcl5Scores||Array(20).fill(-1),B=m(M),U=m(F),j=m(k),G=p(B,s),z=p(U,h),q=C.diagnoses||[],X=q.length>0?q.map(e=>`${e}  ${t.DIAGNOSIS_LABELS[e]||e}`).join("\n"):"[No diagnoses selected]",Y="";Y+=`CLINICAL PSYCHOLOGICAL EVALUATION
...
${D} is a ${n(y.age,"[age]")}-year-old ${n(y.nationality,"[nationality]")} ${y.gender?.toLowerCase()||"individual"} who was referred for a clinical psychological evaluation by ${n(y.referringAttorney,N+" attorney")} in connection with ${N} ${P} case. The purpose of this evaluation is to assess ${D}'s current psychological functioning, document ${N} trauma history, and provide a clinical opinion regarding the psychological impact of the events ${E} experienced.
...
Please note that I am not ${D}'s therapist, and I only met with ${L} for this clinical evaluation. I met with ${L} as an impartial, objective assessor, and I have no vested interest in the outcome of ${N} legal proceedings.
...
`;let Q=[];if(v.siblings&&Q.push(`Siblings: ${v.siblings}${v.birthOrder?` (${v.birthOrder})`:""}`),v.raisedBy&&Q.push(`Raised by: ${v.raisedBy}`),v.parentsWork&&Q.push(`Parents' occupation: ${v.parentsWork}`),v.childhoodLocation&&Q.push(`Childhood location: ${v.childhoodLocation}`),v.childhoodEnvironment&&Q.push(`Childhood environment: ${v.childhoodEnvironment}`),Q.length>0&&(Y+=Q.join("\n")+"\n\n"),v.childhoodStressors&&(Y+=`${v.childhoodStressors}

`),v.travelHistory&&(Y+=`Travel history: ${v.travelHistory}

`),v.faithCommunity||v.currentFaith){let e="";v.faithCommunity&&(e+=v.faithCommunity),v.currentFaith&&(e+=e?`. Currently: ${v.currentFaith}`:v.currentFaith),Y+=`Faith/spiritual background: ${e}

`}v.hobbiesInterests&&(Y+=`Hobbies and interests: ${v.hobbiesInterests}

`),v.personalHistory&&(Y+=`${v.personalHistory}

`),v.familyBackground&&(Y+=`${v.familyBackground}

`),(v.educationLevel||v.degreesAwards||v.educationHistory)&&(Y+=`Education

`,v.educationLevel&&(Y+=`Highest level of education: ${v.educationLevel}
`),v.degreesAwards&&(Y+=`Degrees/Awards: ${v.degreesAwards}
`),(v.educationLevel||v.degreesAwards)&&(Y+="\n"),v.educationHistory&&(Y+=`${v.educationHistory}

`)),(v.employmentHistory||v.currentEmployment)&&(Y+=`Employment

`,v.employmentHistory&&(Y+=`${v.employmentHistory}

`),v.currentEmployment&&(Y+=`Current employment: ${v.currentEmployment}

`)),(v.maritalStatus||v.relationshipHistory||v.childrenInfo)&&(v.maritalStatus&&(Y+=`Marital status: ${v.maritalStatus}
`),v.numberOfChildren&&(Y+=`Children: ${v.numberOfChildren}
`),(v.maritalStatus||v.numberOfChildren)&&(Y+="\n"),v.relationshipHistory&&(Y+=`${v.relationshipHistory}

`),v.childrenInfo&&(Y+=`${v.childrenInfo}

`)),v.currentLiving&&(Y+=`Current living situation: ${v.currentLiving}

`),v.futureHopes&&(Y+=`Future hopes: ${v.futureHopes}
...
`;let V=[];A.appearance&&V.push(A.appearance),A.behavior&&V.push(A.behavior),A.speech&&V.push(`${R} speech was ${A.speech.toLowerCase()}.`),A.mood&&V.push(`${R} mood appeared ${A.mood.toLowerCase()}.`),A.affect&&V.push(`${R} affect was ${A.affect.toLowerCase()}.`),A.thoughtProcess&&V.push(`Thought process: ${A.thoughtProcess}.`),A.thoughtContent&&V.push(`Thought content: ${A.thoughtContent}.`),A.perceptions&&V.push(`Perceptions: ${A.perceptions}.`),A.cognition&&V.push(`Cognition: ${A.cognition}.`),A.insight&&V.push(`Insight: ${A.insight}.`),A.judgment&&V.push(`Judgment: ${A.judgment}.`),A.rapport&&V.push(`Rapport: ${A.rapport}.`),V.length>0?Y+=V.join(" ")+"\n\n":Y+=`${D} is a ${n(y.age,"[age]")}-year-old self-identified ${y.gender?.toLowerCase()||"individual"} who arrived at the appointment on time and was neatly groomed. ${I} was oriented to person, place, time, and situation. There was no evidence of impaired thought process, and ${E} did not appear to respond to internal stimuli or exhibit other symptoms indicative of psychosis.
...
After thorough assessment including clinical interview${B>0||U>0||j>0?" and review of three self-assessment scales":""}, I have concluded the following:

${D} is a reliable reporter, and ${N} account is highly credible.

${R} current psychological symptoms include ${S.currentSymptoms?S.currentSymptoms.substring(0,200)+(S.currentSymptoms.length>200?"...":""):"[symptoms summary]"}. ${I} ${q.length>0?"fully meets":"may meet"} criteria for the following psychological disorder(s):

${X}

`,T.clinicalImpression&&(Y+=`${T.clinicalImpression}

`),Y+=`${D}'s psychological distress has resulted primarily from the trauma ${E} suffered and ${N} fear of returning to ${O}.

Due to ${D}'s traumatic experiences, ${N} psychological functioning has been compromised. If faced with significant stressors, ${E} is at high risk of worsening symptoms which could necessitate a higher level of care.

RECOMMENDATIONS

To improve symptoms and functioning, I recommend that:

${D} be allowed to remain in the United States to reduce ${N} fear of persecution and improve ${N} psychological functioning and well-being.
...
`:Y+=`${D}'s personal strengths and resiliency are substantial. If allowed to remain in the safety of the United States, it is likely that ${N} symptoms and functioning will significantly improve, allowing ${L} to continue making contributions to ${N} family and community.
...
`;let W=M.some(e=>e>=0)||F.some(e=>e>=0)||k.some(e=>e>=0);return W&&(Y+=`ADDENDUM 1: SELF-ASSESSMENT SCALES

`,k.some(e=>e>=0)&&(Y+=`1) PTSD Checklist for DSM-5 (PCL-5)

The first scale completed by ${D} was the PCL-5, which was developed by the National Center for PTSD to assess individuals who experience traumatic events. Here, the respondent rates the severity of 20 different trauma-related symptoms. ${D} was asked to scale to what degree each symptom disturbed or impaired ${L} over the past month. Symptoms are rated from 0 to 4, with 0 indicating "not at all," 1 indicating "a little bit," 2 indicating "moderately," 3 indicating "quite a bit," and 4 indicating "extremely."
...
`),M.some(e=>e>=0)&&(Y+=`2) Patient Health Questionnaire-9 (PHQ-9)
...
${g(M,a,r,D,3)}
Scoring: 0–4 (none-minimal); 5–9 (mild); 10–14 (moderate); 15–19 (moderately severe); 20–27 (severe). ${D}'s score of ${B} out of 27 indicates that ${E} is suffering from ${G} depression.
...
${g(F,l,c,D,3)}
Scoring: 0–4 (none-minimal); 5–9 (mild); 10–14 (moderate); 15–21 (severe). ${D}'s score of ${U} out of 21 indicates that ${E} is suffering from ${z} anxiety.

`)),Y+=`ADDENDUM ${W?"2":"1"}: IMPACT OF TRAUMA ON MEMORY
...
\\f0\\fs24\\cf2 ${r}
}`],{type:"application/rtf"}),{saveAs:l}=await e.A(42191);l(s,a)}e.s(["buildPDFHTML",0,v,"buildReportText",0,f,"generateDOCX",0,$,"generateDOCXBlob",0,y,"generatePDF",0,w,"generatePages",0,S])},42191,e=>{e.v(t=>Promise.all(["static/chunks/034470bgw0i-w.js"].map(t=>e.l(t))).then(()=>t(78631)))}]);